...
import { ArrowLeft, Database, LockKeyhole, Plus, Search } from 'lucide-react';
...
const schemas = {
  ...
  camiones: { title: 'Camiones', singular: 'camión', fields: [{ key: 'placa', label: 'Placa', required: true }, { key: 'anio_modelo', label: 'Año modelo', type: 'number', required: true }, { key: 'aseguradora_rc', label: 'Aseguradora RC', required: true }, { key: 'poliza_rc', label: 'Póliza RC', required: true }] },
...
export default function DataRegistry() {
  ...
  // Estos campos los genera la base de datos o identifican registros protegidos;
  // nunca deben aparecer como campos capturables por el usuario.
  const generatedFieldKeys = new Set(['id', 'created_at', 'updated_at', 'deleted_at', 'created_by', 'updated_by', 'user_id', 'is_default', 'isDefault', 'readonly', 'origen']);
  const formFields = schema.fields.filter((field) => !generatedFieldKeys.has(field.key) && !field.default && !field.generated);
  ...
  const isProtectedRecord = (record) => Boolean(record.is_default || record.isDefault || record.readonly || record.origen === 'sistema');
